...
__global__ void renderKernel(unsigned char* ptr) {
    ...
    for (int i = 0; i < samples; i++) {
        ...
        float ix, iy;
        getInfinityPos(t, ix, iy);
        ix *= 0.8f; // Scale symbol
        iy *= 0.8f;
    ...
    // Glow calculation: 1 / distance
    float glow = 0.015f / (minDist + 0.005f);
    
    // 3. COLORING (Cyan to Magenta gradient)
